...
protected:
    void RunProfile(uint32_t payload_bytes, uint64_t messages,
        ...
        std::cout << "transport=zmq-ipc socket=pubsub codec=raw profile=" << tag
...
TEST_F(ZmqIpcMpStressTest, SmallPayloadHwm1024) {
    RunProfile(256, 20000, 32, "s256h1k", 1024);
}

TEST_F(ZmqIpcMpStressTest, MediumPayloadHwm1024) {
    RunProfile(64u * 1024u, 2000, 32, "m64kh1k", 1024);
}

TEST_F(ZmqIpcMpStressTest, SmallPayloadHwmMessages) {
    RunProfile(256, 20000, 32, "s256hn", 20000);
}

TEST_F(ZmqIpcMpStressTest, MediumPayloadHwmMessages) {
