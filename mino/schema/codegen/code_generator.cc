...
std::string CppStringExpression(std::string_view value) {
    return "std::string(" + CppStringLiteral(value) + ", " +
           std::to_string(value.size()) + "u)";
}
...
void EmitDescriptorConstruction(const SchemaDescriptor& descriptor,
    ...
    for (const FieldDescriptor& field : descriptor.aggregate().fields()) {
        ...
        if (field.default_value().has_value()) {
            default_value =
                std::string("std::optional<::mino::schema::DefaultValue>{") +
                "::mino::schema::DefaultValue(" +
                DefaultKindExpression(field.default_value()->kind()) + ", " +
                CppStringExpression(field.default_value()->canonical_value()) +
                ")}";
        }
    ...
    Line(source, "            " +
                     CppStringExpression(descriptor.canonical_schema()) + ",");
...
Result<GeneratedArtifacts> CodeGenerator::Generate(
    ...
    try {
        ...
        if (options.emit_wire_adapter) {
            ...
            Line(artifacts.source, "#include <string>");
            Line(artifacts.source);
            Line(artifacts.source, "#include \"mino/schema/registry.h\"");
        }
        Line(artifacts.source);
