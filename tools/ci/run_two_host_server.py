#!/usr/bin/env python3
"""Run the server role using the real Mino two-host probe binary."""
...
def main() -> int:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("two-host/server/manifest.json"))
    parser.add_argument("--log", type=Path, default=Path("two-host/server/server.log"))
    ...
    if not args.advertise_address:
        parser.error("--advertise-address is required")
    try:
        return run_role(
            role="server",
            binary=args.binary,
