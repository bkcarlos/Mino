...
| Extended MPMC TSAN | PASS | commit `293e341ee5b078bb324015ee7501559f0fcc6f30`；run `31870648278` attempt `1`；3,299 秒 timed phase；682,761,784 tokens；artifact `extended-mpmc-tsan-31870648278-1`；manifest SHA-256 `3dc49c02277e18aa3ae39fbbf0c4c5753bb5b26497985edbe5ef6bcb7fccc091` | 已关闭 |
...
补充软件门禁：commit `e53e1711` 的 Extended Fuzz（run `31496213551`）、Schema Extended（run `31496213528`）和 Formal Validation（run `31496213565`）均通过；Extended Long-Running run `31859685079` 的 Subscriber/Broadcast、Bridge reconnect 与 Storage job 通过。MPMC TSAN job 暴露 timed phase 逐 token 留存导致内存随时长增长以及 runner 未预留 Bazel/校验收尾时间的问题；修复已包含在 commit `293e341e`，本地 30 秒 target、300 秒 runner 与远程 3,299 秒 timed phase 均通过。该 commit 的 Extended Fuzz run `31870642701`（ASAN/UBSAN × frame/handle/segment 六个一小时 campaign）、Schema Extended run `31870642859`、Formal Validation run `31870642737` 和 Extended Long-Running run `31870648278` 均通过。CI run `31870642736` 发现 `mpsc_channel_test` 在全仓 TSAN 并行调度下耗尽 900 秒，单 target TSAN 15/15 用例仅耗时 184 ms；commit `d8f77b762977f78c8d4a8f37a53c705ca9dffad0` 将该 128-publisher 高线程矩阵标记为 Bazel `exclusive` 后，本地完整 TSAN 137/137 通过，远程 CI run `31875775014` 的 debug/release/ASAN/UBSAN/TSAN/Hermetic LLVM/buildifier/tooling smoke 全部通过。
