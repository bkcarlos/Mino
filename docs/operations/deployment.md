...
(`//mino/runtime/deployment:shared_host_domain`, layout ABI v3 / magic
`MINOSHD3`). SharedHostDomain supports Broadcast (1:N) and MPSC (N:1)
topics with fail-closed schema/QoS/mode matching, multi-publisher lease
recovery via `Recover()`, and a zero-copy borrow poll path over
CentralSlab + AllocationJournal + ShmPinTable (SimpleNode patterns;
incompatible with v1/v2 segments). `LocalBusDeployment`
